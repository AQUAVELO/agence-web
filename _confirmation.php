...
function sendThankYouEmail($toEmail, $prenom, $nom, $telephone, $achat, $montant, $codeValidation) {
    ...
    try {
        ...
        $mail->Body = "<p>Bonjour <strong>$prenom $nom</strong>,</p>
        ...
        $adminMail->Body = "<ul>
            ...
            <li>Code validation : $codeValidation</li>
        </ul>";
        $adminMail->send();
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validateMAC($_POST, MONETICO_KEY)) {
        ...
        if (!empty($_POST['reference'])) {
            ...
            $stmt = $database->prepare("UPDATE formule SET vente = 1, code = ? WHERE reference = ?");
            $stmt->execute([$codeValidation, $reference]);
        }

        if ($email) {
            sendThankYouEmail($email, $prenom, $nom, $telephone, $achat, $montant, $codeValidation);
        }
