...
$menu_query = $database->prepare('SELECT petit_dejeuner, repas_midi, souper, calories, collation FROM menus WHERE jour_du_mois = :jour_du_mois');
$menu_query->bindParam(':jour_du_mois', $jour_du_mois, PDO::PARAM_INT);
$menu_query->execute();
$menu_data = $menu_query->fetch(PDO::FETCH_ASSOC);


// Définir le jour que vous voulez, par exemple le jour 15
$jour_actuel = 15;
...
while ($row = $photo_query->fetch(PDO::FETCH_ASSOC)) {
    // Stocker la photo dans le tableau associatif avec la clé du repas
